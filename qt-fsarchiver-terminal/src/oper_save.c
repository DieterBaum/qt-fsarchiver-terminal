    ...
    *filecost=FSA_COST_PER_FILE; // fixed cost per file
    ...
    if (meldeflag  == 0) {
           // Anzeige im Terminal 
            ...
            fp = fopen(home, "w");
            fprintf(fp, "%.9lld \r", anzahlfile); //Anzahl der zu sichernden Dateien
            fclose(fp);
            } 
    	if (meldeflag  == 1 ) { 
            // Anzeige im Terminal
            ...
            if (save->objectid <= 1000)  
               printf("     %.5lld            %.0f%%             \r ",save->objectid,progress); 
           strcpy(home, home1); 
           strcat(home, filename1);
           FILE *fp1;
           fp1 = fopen(home, "w");
           fprintf(fp1, "%.0f%% \r", progress);
           fclose(fp1);
           anzahlfile = save->objectid;
          // if(anzahlfile > 0)
          // {
           strcpy(home, home1); 
           strcat(home, filename2);
           FILE *fp;
           fp = fopen(home, "w");
           fprintf(fp, "%.9lld \r", anzahlfile);
           fclose(fp);
          // } 
       }
...
int oper_save(char *archive, int argc, char **argv, int archtype)
{
    ...
    meldeflag = 1;
    char home[100];
    char filename[100] = ".config/qt-fsarchiver/meldung.txt";
    strcpy(home, home1);
    strcat(home, filename);
    FILE *fp;
    fp = fopen(home, "w");
